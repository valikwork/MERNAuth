...
export default function GamesScreen() {
  ...
  const fetchGames = (currentPage, currentGenre) => fetch(MIRAPLAY_API_ROUTE, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      ...
      "genre": currentGenre === 'ALL' ? false : currentGenre,
      "gamesToShow": GAMES_TO_SHOW
    })
  }).then((res) => res.json())

  const {data, error, isLoading, refetch} = useQuery({ 
    queryKey: ['games', currentPage, currentGenre], 
    queryFn: () => fetchGames(currentPage, currentGenre),
  })
